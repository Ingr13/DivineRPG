...
public class WorldGenUviteTree implements IWorldGenerator
{
    @Override
    public void generate(Random var1, int var2, int var3, World var4, IChunkProvider var5, IChunkProvider var6)
    {
        switch (var4.provider.dimensionId)
        {
            ...
            case 4:
                this.generateUvite(var4, var1, var2 * 16, var3 * 16);

            default:
        }
    }

    public void generateUvite(World world, Random rand, int chunkX, int chunkZ) {
        BiomeGenBase biome = world.getWorldChunkManager().getBiomeGenAt(chunkX, chunkZ);
        WorldGenEnergyTrees tree = new WorldGenEnergyTrees();

        if((biome instanceof BiomeGenUvite))// then add ||BiomeGenXYZ if you want more.
        {
            for(int x = 0; x < 6; x++)
            {
