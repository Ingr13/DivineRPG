...
public class WorldGenAzuriteTree implements IWorldGenerator
{
    @Override
    public void generate(Random var1, int var2, int var3, World var4, IChunkProvider var5, IChunkProvider var6)
    {
        switch (var4.provider.dimensionId)
        {
            ...
            case 3:
                this.generateAzurite(var4, var1, var2 * 16, var3 * 16);

            default:
        }
    }

    public void generateAzurite(World world, Random rand, int chunkX, int chunkZ) {
        BiomeGenBase biome = world.getWorldChunkManager().getBiomeGenAt(chunkX, chunkZ);
        WorldGenAzuriteTrees tree = new WorldGenAzuriteTrees();

        if((biome instanceof BiomeGenAzurite))// then add ||BiomeGenXYZ if you want more.
        {
            for(int x = 0; x < 6; x++)
            {
