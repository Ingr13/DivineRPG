...
public class ArmorEffectHandler  {
    ...
    public void onLivingHurtEvent(LivingHurtEvent e) {
		...
		if (bootID == OverworldItemHelper.netheriteBoots.itemID
					...
					&& helmID == OverworldItemHelper.netheriteHead.itemID)
			{
		...
		//Inferno
					...
					&& helmID == OverworldItemHelper.infernoHead.itemID)
			{
		...
		//Bedrock
					...
					&& helmID == OverworldItemHelper.bedrockHead.itemID)
			{
				if (s == DamageSource.explosion || s == DamageSource.explosion2 ||
		...
		{
			if (e.entityLiving.worldObj.provider.dimensionId == DimensionRegistry.IceikaID)
			{
				e.ammount *= 0.2;
			}
		}
